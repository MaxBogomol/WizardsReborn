...
public class AirChargeSpell extends ChargeSpell {
    ...
    public void onImpact(Level level, SpellEntity entity, RayHitResult hitResult, Entity target) {
        ...
        if (!entity.level().isClientSide()) {
            ...
            float damage = (3.0f + (focusLevel * 1.5f)) + (magicModifier * 2f) + WizardsRebornConfig.SPELL_CHARGE_DAMAGE.get().floatValue() + WizardsRebornConfig.AIR_CHARGE_DAMAGE.get().floatValue();
