...
public class FrostChargeSpell extends ChargeSpell {
    ...
    public void onImpact(Level level, SpellEntity entity, RayHitResult hitResult, Entity target) {
        ...
        if (!entity.level().isClientSide()) {
            ...
            float damage = (4.5f + (focusLevel * 1.5f)) + (magicModifier * 2f) + WizardsRebornConfig.SPELL_CHARGE_DAMAGE.get().floatValue() + WizardsRebornConfig.FROST_CHARGE_DAMAGE.get().floatValue();
