    ...
    public void onImpact(Level level, SpellEntity entity, RayHitResult hitResult, Entity target) {
        ...
        if (!level.isClientSide()) {
            if (target instanceof LivingEntity livingEntity) {
                ...
                float damage = (1.5f + (focusLevel * 0.5f)) + magicModifier + WizardsRebornConfig.SPELL_PROJECTILE_DAMAGE.get().floatValue() + WizardsRebornConfig.HOLY_PROJECTILE_DAMAGE.get().floatValue();
