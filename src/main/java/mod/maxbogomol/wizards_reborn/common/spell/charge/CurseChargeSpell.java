    ...
    public void onImpact(Level level, SpellEntity entity, RayHitResult hitResult, Entity target) {
        ...
        if (!entity.level().isClientSide()) {
            if (target instanceof LivingEntity livingEntity) {
                ...
                float damage = (2.5f + (focusLevel * 1.5f)) + (magicModifier * 2f) + WizardsRebornConfig.SPELL_CHARGE_DAMAGE.get().floatValue() + WizardsRebornConfig.CURSE_CHARGE_DAMAGE.get().floatValue();
