...
public class OpenAlchemyBagPacket {
    public OpenAlchemyBagPacket() {

    }

    public static OpenAlchemyBagPacket decode(FriendlyByteBuf buf) {
    ...
    public static void handle(OpenAlchemyBagPacket msg, Supplier<NetworkEvent.Context> ctx) {
        if (ctx.get().getDirection().getReceptionSide().isServer()) {
            ctx.get().enqueueWork(() -> {
                ...
                for (ItemStack item : items) {
                    if (item.getItem() instanceof AlchemyBagItem stack) {
